...
100.times do
    r = Review.create(user_id: User.all.sample.id, title: Faker::Movie.quote, rate_up: Faker::Number.between(from: 1, to: 50), rate_down: Faker::Number.between(from: 1, to: 50))
    ...
    g = Get.create(course_id: Course.all.sample.id, review_id: r.id)
end